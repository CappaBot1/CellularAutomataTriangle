...
function render({ id, numColours, size, colours }) {
    if (size % 2 !== 0) throw "canvas not even!!!";
    if (size % 4 !== 0) throw "canvas not 4ven!!!";

    if (numColours < colours.length - 1) throw "too many colours!!!";
    if (numColours > colours.length - 1) throw "not enough colours!!!";
...
//const size = 200*4;
const size = (x => 2*3**x+2)(5); // use this function for sierpinski triangles
